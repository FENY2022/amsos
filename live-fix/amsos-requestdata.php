...
if (!isset($conn) || !($conn instanceof mysqli) || $conn->connect_errno) {
    error_log('AMSOS DB connection failed in live-fix/amsos-requestdata.php: ' . (isset($conn) && $conn instanceof mysqli ? $conn->connect_error : 'No mysqli connection'));
...
$query = "
    SELECT srf.*, MAX(srffeedback.feedback) AS rate
    FROM srf
                            ...
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php $count = 1; while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td><span class="badge bg-secondary"><?php echo h($row['ticketNumber'] ?? ''); ?></span></td>
                                        <td><?php echo !empty($row['date']) ? h(date('M j, Y', strtotime($row['date']))) : ''; ?></td>
                                        <td><strong><?php echo h($row['name'] ?? ''); ?></strong><br><small class="text-muted"><?php echo h($row['position'] ?? ''); ?></small></td>
                                        <td><?php echo h($row['office'] ?? ''); ?></td>
                                        <td><span class="badge bg-info"><?php echo h($row['requestType'] ?? ''); ?></span></td>
                                        <td><?php echo getStarRating($row['rate'] ?? ''); ?></td>
                                        <td>
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-xl">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"><i class="bi bi-file-earmark-text me-2"></i>View Document - Ticket #<?php echo h($row['ticketNumber'] ?? ''); ?></h5>
