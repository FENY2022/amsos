...
if (!isset($conn) || !($conn instanceof mysqli) || $conn->connect_errno) {
    error_log('AMSOS DB connection failed in live-fix/print-all-requestdata.php: ' . (isset($conn) && $conn instanceof mysqli ? $conn->connect_error : 'No mysqli connection'));
...
$query = "
    SELECT srf.id, srf.ticketNumber, srf.date, srf.name, srf.office, srf.requestType, MAX(srffeedback.feedback) AS rate
    FROM srf
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <section class="document-card">
                        <div class="document-head no-print">
                            <div>
                                <h2>Ticket #<?php echo h($row['ticketNumber'] ?? ''); ?></h2>
                                <small><?php echo h($row['name'] ?? ''); ?> | <?php echo h($row['office'] ?? ''); ?> | <?php echo h($row['requestType'] ?? ''); ?></small>
                            </div>
                            <span class="badge-soft"><?php echo h(ratingLabel($row['rate'] ?? '')); ?></span>
                        </div>
